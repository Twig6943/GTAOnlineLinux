	...
	for userID in userIDs:
		...
		with open(os.path.expanduser(f'~/.steam/root/userdata/{userID}/config/localconfig.vdf'), 'r') as steamConfig:
			...
			for index, line in enumerate(lines):
				if line.strip() == f'\"{gtaLegacySteamID}\"' and lines[index + 1].strip() == '{':
					inGTA = True
					gotGTA = True
				if inGTA:
...
def writeHosts() -> None:
	with open('/etc/hosts', 'r') as hostsFile:
		...
		if '0.0.0.0 paradise-s1.battleye.com\n' not in lines:
			toAppend += '0.0.0.0 paradise-s1.battleye.com\n'
		hostsFile.close()
	if toAppend == '':
		return
...
def createStartupMeta() -> None:
	path = getSteamGamePath(gtaLegacySteamID, 'gta legacy')
	path += '/steamapps/common/Grand Theft Auto V/x64/data/startup.meta'
	with open(path, 'w') as metaFile:
		metaFile.write(metaContent)
		metaFile.close()

print("""\
GTAOnlineLegacyLinux Copyright (C) 2026  WerIstLuka <weristluka@gmail.com>
